...
place_right_up = ([10, -20, 25, 0, 90, 90])  # Right position for defective bottles
place_right_down = ([10, -20, 9, 0, 90, 90])
place_left_up = ([10, 20, 25, 0, 90, 90])  # Left position for non-defective bottles
place_left_down = ([10, 20, 9, 0, 90, 90])
...
while catch_count < max_catch_count:
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence > 0.2:  # Adjust confidence threshold
            ...
            if CLASSES[idx] == "bottle":
                ...
                # Calculate pick position dynamically
                pick_position = [
                ...
                # Place bottle based on classification
                if label == "Defective":
                    # Move to right place position
                    robot.move_pose(*convert_data(place_right_up))
                    time.sleep(1)
                    robot.move_pose(*convert_data(place_right_down))
                else:
                    # Move to left place position
                    robot.move_pose(*convert_data(place_left_up))
                    time.sleep(1)
                    robot.move_pose(*convert_data(place_left_down))
    ...
    catch_count += 1
    print(f"Processed object count: {catch_count}")
